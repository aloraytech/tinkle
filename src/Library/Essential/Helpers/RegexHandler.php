...
class RegexHandler
{
   ...
   public const REGEX_NUMERIC = '/^(\w+|\d+)$/';
   public const REGEX_WORD = '/^(\w+|\d+)$/';
   public const REGEX_ALPHA_NUMERIC = '/^(\w+|\d+)$/';
   public const REGEX_DATE = '/^[0-9]{1,2}[-/][0-9]{1,2}[-/][0-9]{4}$/';
   public const REGEX_MONEY = '^[0-9.,]*(([.,][-])|([.,][0-9]{2}))?$';
   ...
   public const REGEX_MOBILE = '/^[-]?[0-9]+[.]?[0-9]+$/';



    public function verify(string $subject,$type){
        if ((preg_match($type, $subject) === 1)) {
            return true;
        } else {
            return false;
        }
